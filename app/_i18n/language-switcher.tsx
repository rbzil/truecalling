...
export function LanguageSwitcher() {
  ...
  const switchTo = (next: Locale) => {
    ...
    const target = switchLocaleOnPath(pathname || `/${locale}`, next);
    router.push(target);
    setOpen(false);
  };
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <span className="text-[14px] leading-none">{currentFlag}</span>
            ...
            {locales.map((l) => {
              ...
              return (
                <li key={l} role="option" aria-selected={active}>
                  ...
                  >
                    <span className="text-[16px] leading-none">{localeFlags[l]}</span>
                    <span className="flex-1 truncate font-medium">{localeNames[l]}</span>
                    {active && (
